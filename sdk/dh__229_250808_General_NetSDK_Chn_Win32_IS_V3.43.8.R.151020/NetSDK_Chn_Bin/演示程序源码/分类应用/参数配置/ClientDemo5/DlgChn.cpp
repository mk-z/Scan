...
BOOL CDlgChn::SetChnState(DWORD dwState, int nLen /* = 32 */)
{
	if(nLen > 32)
	{
		return FALSE;
	...
	for(UINT uID = uIDBase; uID < uIDBase+nLen; uID++)
	{
		bCheck = (dwState>>(uID-uIDBase))&0x01 > 0 ? TRUE : FALSE;
		((CButton *)GetDlgItem(uID))->SetCheck(bCheck);
	}
...
BOOL CDlgChn::GetChnState(DWORD* dwState, int nLen)
{
	if(nLen > 32 || dwState == NULL)
	{
		return FALSE;
	}
	*dwState = 0;
