...
void CAlarmVLostDlg::InitDlgInfo(DHDEV_ALARM_SCHEDULE_EX *pAlarmInfo, BYTE nAlarmIn, BYTE nAlarmOut, int nChannel, vector<string>& vecAlarmName)
{
	if(pAlarmInfo != NULL)
	{
		...
		m_nChannel = nChannel;	
		m_pstVideoLostInfo = new CFG_VIDEOLOST_INFO[nChannel/*CHANNEL_NUM_LIMIT*/];
		if (m_pstVideoLostInfo == NULL)
		{
			return;
		}
		memset(m_pstVideoLostInfo, 0, nChannel*sizeof(CFG_VIDEOLOST_INFO));
